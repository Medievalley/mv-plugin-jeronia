...
public class ProjectContextImpl extends BaseContext implements ProjectContext {
    ...
    @Override
    public Optional<StorageModel> getStorage(int mainStructId) throws Exception {
        try {
            QueryRunner run = new QueryRunner(getDataSource());
            ResultSetHandler<StorageModel> h = new BeanHandler<>(StorageModelImpl.class);
            StorageModel storage = run.query(String.format("SELECT id, resources, deposit from struct WHERE type_id=%d", mainStructId), h);
