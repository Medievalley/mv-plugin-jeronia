...
import org.shrigorevich.ml.domain.admin.DraftNpc;
import org.shrigorevich.ml.domain.npc.*;
...
public class NpcServiceImpl extends BaseService implements NpcService {
    ...
    public void commitNpc(DraftNpc npc) throws Exception {
        try {
            int id = context.save(
                npc.getName(), npc.getRole().getId(), npc.getStructId(), npc.getWorkLoc(), npc.getSpawnLoc());
            setup(id);
        } catch (Exception ex) {
